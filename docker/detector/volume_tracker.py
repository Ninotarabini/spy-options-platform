...
class VolumeTracker:
    ...
    def calculate_deltas(self, calls_volume: int, puts_volume: int) -> tuple:
        """
        Calcula delta (incremento) desde último scan.
        
        Args:
            calls_volume: Volumen total actual de calls
            puts_volume: Volumen total actual de puts
            
        Returns:
            (calls_delta, puts_delta)
        """
        # Primer scan: delta = volumen actual
        if self.first_scan:
